...
func (instance Event) GetEventByArticleId(articleId uuid.UUID, userId uuid.UUID) (*event.Event, error) {
	...
	var newsletterArticleDto dto.Article
	err = postgresConnection.Get(&newsletterArticleDto, queries.Article().Select().NewsletterArticleByArticleId(),
		articleId)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		log.Errorf("Error retrieving newsletter article data for article %s from the database: %s", articleId,
			err.Error())
		return nil, err
	}
	...
	if userId != uuid.Nil {
		eventArticles := append([]interface{}{}, eventData.Id, newsletterArticleDto.Id)
	...
	for _, agendaItemData := range agendaItemsData {
		...
		if agendaItemData.VotingArticleId != uuid.Nil {
			...
			articleSituation, err := articlesituation.NewBuilder().
				Result(agendaItemArticles[agendaItemData.VotingArticleId].Voting.Result).
				ResultAnnouncedAt(agendaItemArticles[agendaItemData.VotingArticleId].Voting.ResultAnnouncedAt).
			...
			articleDomain, err = articleBuilder.
				...
				Content(agendaItemArticles[agendaItemData.VotingArticleId].Voting.Description).
	...
	var relatedArticles []article.Article
	if newsletterArticleDto.Id != uuid.Nil {
		articleType, err := articletype.NewBuilder().
			Id(newsletterArticleDto.ArticleType.Id).
			Description(newsletterArticleDto.ArticleType.Description).
			Codes(newsletterArticleDto.ArticleType.Codes).
			Color(newsletterArticleDto.ArticleType.Color).
			Build()
		if err != nil {
			log.Errorf("Error validating data for article type %s of article %s: %s",
				newsletterArticleDto.ArticleType.Id, newsletterArticleDto.Id, err.Error())
			return nil, err
		}

		articleBuilder := article.NewBuilder().
			Id(newsletterArticleDto.Id).
			AverageRating(newsletterArticleDto.AverageRating).
			NumberOfRatings(newsletterArticleDto.NumberOfRatings).
			Type(*articleType).
			CreatedAt(newsletterArticleDto.CreatedAt).
			UpdatedAt(newsletterArticleDto.UpdatedAt)

		if _, exists := userArticles[newsletterArticleDto.Id]; exists {
			articleBuilder.UserRating(userArticles[newsletterArticleDto.Id].Rating).
				ViewLater(userArticles[newsletterArticleDto.Id].ViewLater)
		}

		newsletterArticle, err := article.NewBuilder().
			Id(newsletterArticleDto.Id).
			Title(fmt.Sprint("Boletim do dia ",
				newsletterArticleDto.Newsletter.ReferenceDate.Format("02/01/2006"))).
			Content(newsletterArticleDto.Newsletter.Description).
			AverageRating(newsletterArticleDto.AverageRating).
			NumberOfRatings(newsletterArticleDto.NumberOfRatings).
			Type(*articleType).
			CreatedAt(newsletterArticleDto.CreatedAt).
			...
			Build()
		if err != nil {
			log.Errorf("Error validating data for article %s: %s", newsletterArticleDto.Id, err.Error())
			return nil, err
		}

		relatedArticles = append(relatedArticles, *newsletterArticle)
	}
