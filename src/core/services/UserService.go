...
func (instance User) ResendUserAccountActivationEmail(userId uuid.UUID) error {
	...
	if err != nil {
		log.Errorf("Error setting new account activation code for user %s: %s", userData.Id(), err.Error())
		return nil
	}

	userData, err = instance.userRepository.UpdateUser(*userData)
	if err != nil {
		log.Errorf("Error updating account activation code for user %s: %s", userData.Id(), err.Error())
		return err
	}

	err = instance.emailService.SendUserAccountActivationEmail(*userData)
	if err != nil {
		log.Errorf("Error resending account activation email for user %s: %s", userData.Id(), err.Error())
	}
...
func (instance User) ActivateUserAccount(userAccountActivationData user.User) (*user.User, error) {
	...
	if err != nil {
		log.Errorf("Error building the account activation role for user %s: %s", userData.Id(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("Error setting the account activation role for user %s: %s", userData.Id(), err.Error())
		return nil, err
	}

	userData, err = instance.userRepository.UpdateUser(*userData)
	if err != nil {
		log.Errorf("Error activating account for user %s in the database: %s", userData.Id(), err.Error())
		return nil, err
	}

	err = instance.sessionRepository.DeleteSessionsByUserId(userData.Id())
	if err != nil {
		log.Errorf("Error deleting session for user %s: %s", userData.Id(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("Error generating tokens for user %s: %s", userData.Id(), err.Error())
		return nil, err
	}

	err = instance.sessionRepository.CreateSession(userData.Id(), sessionId, userData.AccessToken(), userData.RefreshToken())
	if err != nil {
		log.Errorf("Error creating session for user %s: %s", userData.Id(), err.Error())
		return nil, err
	}
